...
export type ShowFormSchema = z.infer<typeof schema>;

interface ShowFormProps {
  initial?: TvShow | null;
  genres: Genre[];
  submitting: boolean;
  onSubmit: (values: ShowFormSchema) => void;
  onCancel: () => void;
}

export function ShowForm({ initial, genres, submitting, onSubmit, onCancel }: ShowFormProps) {
  const { register, handleSubmit, setValue, watch, formState: { errors } } = useForm<ShowFormSchema>({
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Input label="Backdrop URL" {...register('backdrop_url')} />
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {genres.map((g) => (
            <button key={g.id} type="button" onClick={() => toggleGenre(g.id)} className={'rounded-full border px-3 py-1 text-xs font-medium ' + (genreIds.includes(g.id) ? 'border-primary bg-primary/15 text-primary' : 'border-borderc text-text-secondary hover:bg-surface-hover')}>
              {g.name}
            </button>
          ))}
        </div>
      ...
      <div className="flex justify-end gap-3 border-t border-borderc pt-4">
        ...
        <Button type="submit" loading={submitting}>{initial ? 'Save changes' : 'Create show'}</Button>
      </div>
    </form>
  );
}
